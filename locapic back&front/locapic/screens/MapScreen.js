...
export default function MapScreen() {
  ...
  useEffect(() => {
    ...
    const listPOIMArker = async () => {

      AsyncStorage.getItem("listPOI", 
        function(error, data){

          // envoie le marker dans le store
          var StorePOI = JSON.parse(data)
          console.log('listPOI',StorePOI);
          setListPOI(StorePOI)
  ...
  var handleSubmit = () => {
    // faire une copie de listPOI car la modification de listPOI est asynchrone :
    var userListPOI = [...listPOI, {longitude: tempPOI.longitude, latitude: tempPOI.latitude, titre: titrePOI, description: descPOI } ]
    setListPOI(userListPOI);

    // enregistrement des POI dans le local storage :
    AsyncStorage.setItem("listPOI", JSON.stringify(userListPOI))
    setIsVisible(false);
    setTempPOI();
    setDescPOI();
    setTitrePOI();
